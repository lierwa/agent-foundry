...
```bash
node "/Users/junxi/Desktop/work/agent-foundry/.codex/get-shit-done/bin/gsd-tools.cjs" validate health $REPAIR_FLAG
```
...
```bash
node "/Users/junxi/Desktop/work/agent-foundry/.codex/get-shit-done/bin/gsd-tools.cjs" validate health
```
...
TASKS_DIR="/Users/junxi/Desktop/work/agent-foundry/.codex/tasks"
if [ -d "$TASKS_DIR" ]; then
  STALE_COUNT=$(find "$TASKS_DIR" -maxdepth 1 -type d -mtime +1 2>/dev/null | wc -l)
  if [ "$STALE_COUNT" -gt 0 ]; then
    echo "⚠️  Found $STALE_COUNT stale task directories in /Users/junxi/Desktop/work/agent-foundry/.codex/tasks/"
    echo "   These are leftover from crashed subagent sessions."
    echo "   Run: rm -rf /Users/junxi/Desktop/work/agent-foundry/.codex/tasks/*  (safe — only affects dead sessions)"
  fi
fi
```
